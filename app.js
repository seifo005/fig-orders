(() => {
  const STORAGE_KEY = 'figPreordersV6_Orders';
  const VARS_KEY = 'figVarietiesV6';
  ...
  async function linkVarietiesJson(){
    if(!window.showOpenFilePicker){ alert('ربط الملف متاح على Chrome/Edge.'); return; }
    try{
  ...
  async function linkOrdersJson(){
    if(!window.showOpenFilePicker){ alert('هذه الميزة مدعومة على Chrome/Edge.'); return; }
    try{
      const [handle] = await window.showOpenFilePicker({ types:[{description:'JSON', accept:{'application/json':['.json']}}] });
      const perm = await handle.requestPermission({mode:'readwrite'});
  ...
  function addDraftItem(){
    const kind = $('itemKind').value; // seedling/cutting
    ...
    draftItems.push({ kind, variety, quantity: qty, unitPrice: price, total: qty*price });
    renderDraftItems();
  }
  function renderDraftItems(){
    ...
    draftItems.forEach((it,i)=>{
      sum += it.total||0;
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${i+1}</td><td>${it.kind==='cutting'?'عقلة':'شتلة'}</td><td>${esc(it.variety)}</td><td>${it.quantity}</td><td>${fmt(it.unitPrice)}</td><td><b>${fmt(it.total)}</b></td><td><button class="btn" data-del-item="${i}">حذف</button></td>`;
      body.appendChild(tr);
    });
  ...
  function renderOrdersToPrintable(selected){
    const blocks = selected.map(o => {
      const itemsRows = (o.items||[]).map((it,idx)=>`
        <tr>
          <td>${idx+1}</td>
          <td>${it.kind==='cutting'?'عقلة':'شتلة'}</td>
          <td>${esc(it.variety)}</td>
          <td>${it.quantity}</td>
      ...
      `).join('');
      return `
        <section class="print-page print-box">
          <div class="print-header">
            <div class="print-logo">مشتل التين</div>
            <div class="muted">${new Date(o.createdAt).toLocaleString('ar-DZ')}</div>
          </div>
          <div><b>الزبون:</b> ${esc(o.customerName)} — <b>الهاتف:</b> ${esc(o.phone)}</div>
          <div><b>العنوان:</b> ${esc(o.city||'')} ${esc(o.address||'')}</div>
          <div class="muted">الحالة: ${statusLabel(o.status)} — طريقة التوصيل: ${o.deliveryMethod}</div>
          <div style="margin-top:8px;overflow:auto">
            <table style="width:100%;border-collapse:collapse">
              <thead><tr style="background:#f6f6f6"><th>#</th><th>النوع</th><th>الصنف</th><th>الكمية</th><th>السعر</th><th>الإجمالي</th></tr></thead>
              <tbody>${itemsRows}</tbody>
              <tfoot><tr><td colspan="5" style="text-align:right;font-weight:700">الإجمالي</td><td style="font-weight:700">${fmt(o.total)}</td></tr></tfoot>
            </table>
          </div>
          <div style="margin-top:6px" class="muted">عربون: ${fmt(o.depositDZD||0)} — ملاحظات: ${esc(o.notes||'')}</div>
        </section>
      `;
    }).join('');
    return `<!doctype html><html lang="ar" dir="rtl"><head>
      <meta charset="utf-8"><title>طباعة الحجوزات</title>
      <style>
        body{font-family: system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif; padding:16px}
        .muted{color:#6b7280;font-size:12px}
        .print-box{border:1px solid #ccc;border-radius:12px;padding:12px;margin-bottom:12px}
        ...
        th,td{border-top:1px solid #eee;padding:8px;text-align:right}
  ...
  function printSelected(){
    ...
    const html = renderOrdersToPrintable(selected);
  ...
  function printCurrentDraft(){
    ...
    const html = renderOrdersToPrintable([tmp]);
    const w = window.open('', '_blank'); w.document.write(html); w.document.close(); w.focus(); w.print();
  }
  ...
  function renderList(){
    ...
    filtered.forEach(o=>{
      ...
      tr.innerHTML = `
        ...
        <td>${esc(o.phone)}</td>
        <td>${(o.items||[]).map(i=>`<span class="badge">${i.kind==='cutting'?'عقلة':'شتلة'} — ${esc(i.variety)} ×${i.quantity}</span>`).join(' ')}</td>
  ...
  document.addEventListener('click', (e)=>{
    ...
    if(id==='btn-export-selected'){
      const checks = Array.from(document.querySelectorAll('#ordersBody input[type="checkbox"][data-id]:checked'));
      if(checks.length===0){ alert('اختر عنصرًا واحدًا على الأقل من الجدول.'); return; }
      const ids = new Set(checks.map(ch=> ch.getAttribute('data-id')));
      const sel = orders.filter(o=> ids.has(o.id));
      downloadJSON(`fig-preorders-selection-${Date.now()}.json`, sel);
    } else if(id==='btn-print-selected'){
      printSelected();
    } else if(id==='btn-print-order'){
      printCurrentDraft();
    } else if(id==='btn-export-all'){
      downloadJSON(`fig-preorders-${new Date().toISOString().slice(0,10)}.json`, orders);
    } else if(id==='btn-import'){
      document.getElementById('fileImport').click();
    } else if(id==='btn-link-orders-json'){
      linkOrdersJson();
    } else if(id==='btn-link-varieties-json'){
      linkVarietiesJson();
    }
  });

  let chartVariety, chartDay, chartStatus, chartRevenueMonthly;
  function calcFinance(){
    const totals = {
      sumAll: orders.reduce((s,o)=> s + Number(o.total||0), 0),
      deposits: orders.reduce((s,o)=> s + Number(o.depositDZD||0), 0),
      deliveredAmount: orders.filter(o=>o.status==='delivered').reduce((s,o)=> s + Number(o.total||0), 0),
    };
    totals.outstanding = Math.max(0, totals.sumAll - totals.deposits);
    return totals;
  }
  function renderKPIs(){
    ...
    $('#kpi-delivered-amount').textContent = fmt(F.deliveredAmount);
    $('#kpi-deposits').textContent = fmt(F.deposits);
    $('#kpi-outstanding').textContent = fmt(F.outstanding);
  }
  function renderCharts(){
    ...
    orders.filter(o=>o.status==='delivered').forEach(o=>{
      const d=new Date(o.createdAt); const key = `${d.getFullYear()}-${('0'+(d.getMonth()+1)).slice(-2)}`;
      mapM.set(key, (mapM.get(key)||0) + Number(o.total||0));
    });
    const entriesM = Array.from(mapM.entries()).sort();
    const labelsM = entriesM.map(([k])=> k), dataM = entriesM.map(([_,v])=> v);
    if(chartRevenueMonthly) chartRevenueMonthly.destroy();
    chartRevenueMonthly = new Chart($('#chartRevenueMonthly'), { type:'bar', data:{labels:labelsM, datasets:[{label:'المداخيل (المُسلَّمة)', data:dataM}]}, options:{responsive:true, maintainAspectRatio:false} });
  ...
  document.getElementById('btn-link-orders-json')?.addEventListener('click', linkOrdersJson);
  document.getElementById('btn-link-varieties-json')?.addEventListener('click', linkVarietiesJson);
  $('filter-q').addEventListener('input', renderList);
  $('filter-status').addEventListener('input', renderList);
  document.getElementById('btn-export-all')?.addEventListener('click', ()=> downloadJSON(`fig-preorders-${new Date().toISOString().slice(0,10)}.json`, orders));
  ...
  document.getElementById('btn-print-selected')?.addEventListener('click', printSelected);
  document.getElementById('btn-print-order')?.addEventListener('click', printCurrentDraft);
  ...
  (async ()=>{
    await loadVarieties();
    try{
      const r = await fetch('orders.json', {cache:'no-store'});
      if(r.ok){ const arr = await r.json(); if(Array.isArray(arr)) orders = arr; }
    }catch(e){ /* fallback local */ }
    if(!Array.isArray(orders)) orders = safeGet(STORAGE_KEY, []);
